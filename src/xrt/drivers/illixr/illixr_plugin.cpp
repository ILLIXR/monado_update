...
illixr_plugin::illixr_plugin(const std::string &name_, phonebook *pb_)
    : threadloop{name_, pb_}, pb{pb_}, sb{phonebook_->lookup_impl<switchboard>()},
      ...
      signal_writer_{sb->get_network_writer<data_format::illixr_signal>(
          "offload_signal", network::topic_config{network::topic_config::BOOST, network::topic_config::UDP})}
#ifdef USING_OPENXR
      ,
...
illixr_plugin::_p_thread_setup()
{
	...
	while (!have_cfg) {
		...
		if (cfg != nullptr) {
			...
			printf("     fov left: %f, %f\n", hmd_config_.config.fov_angle_left[0],
			       hmd_config_.config.fov_angle_left[1]);
			printf("     fov left: %f, %f\n", hmd_config_.config.fov_angle_right[0],
			       hmd_config_.config.fov_angle_right[1]);
			printf("     fov left: %f, %f\n", hmd_config_.config.fov_angle_up[0],
			       hmd_config_.config.fov_angle_up[1]);
			printf("     fov left: %f, %f\n", hmd_config_.config.fov_angle_down[0],
			       hmd_config_.config.fov_angle_down[1]);
			printf("     size: %d, %d\n", hmd_config_.config.recommended_image_width,
			       hmd_config_.config.recommended_image_height);
			printf("     ipd: %f\n", hmd_config_.ipd);
			have_cfg = true;
		} else {
	...
	status_ = RUNNING;
	sb->schedule<data_format::ipd>(
	    id_, "current_ipd",
	    [&](const switchboard::ptr<const data_format::ipd> &datum, size_t) { ipd_callback(datum); });
}

void
illixr_plugin::ipd_callback(const switchboard::ptr<const data_format::ipd> &datum)
{
...
void
illixr_plugin::_p_one_iteration()
{
	std::this_thread::sleep_for(100ms);
}

hmd_config
illixr_plugin::get_config(float scale, bool double_width) const
{
	while (status_ != RUNNING)
		std::this_thread::sleep_for(10ms);
	return hmd_config_.get_cfg(scale, double_width);
}
